...
pipeline <- drake_plan(
  ...
  c = target(conform_data(data = dat, control = LDA_control()),
             transform = map(dat)),
  lda = target(LDATS::LDA(data = c,topics = 2:5, replicates = 2),
                                transform = map(c)),
  classic =  target(LDATS::TS(LDAS = lda, formulas = ~1, nchangepoints = c(0, 1), control = TS_control(response = multinom_TS)),
                    transform = map(lda))#,
